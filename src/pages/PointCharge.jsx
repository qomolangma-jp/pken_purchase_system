...
const PointCharge = () => {
  ...
  const applyPresetAmount = (presetAmount) => {
  ...
  const handleSubmit = async () => {
    if (!user) {
      showValidationWarning('ログインが必要です。');
      return;
    }

    if (!normalizedAmount || normalizedAmount < MIN_AMOUNT) {
      showValidationWarning('申請額は100pt以上で入力してください。');
      return;
    }

    if (normalizedAmount >= MAX_AMOUNT) {
      showValidationWarning('申請額は50,000pt未満で入力してください。');
      return;
    }
    ...
    openModal({
      type: 'confirm',
      title: 'ポイントチャージ申請の確認',
      message: `${normalizedAmount.toLocaleString()}ptのチャージ申請を行います。よろしいですか？`,
      confirmText: '確定',
      cancelText: '戻る',
      onConfirm: async () => {
        ...
        try {
          ...
          const response = await fetch(`${API_BASE_URL}/api/charge/apply`, {
            method: 'POST',
            headers: {
          ...
          if (!response.ok) {
            throw new Error(data?.message || 'チャージ申請に失敗しました。');
          }

          openModal({
            type: 'success',
            title: '申請完了',
            message: `${normalizedAmount.toLocaleString()}ptのチャージ申請を受け付けました。`,
            confirmText: 'マイページへ',
